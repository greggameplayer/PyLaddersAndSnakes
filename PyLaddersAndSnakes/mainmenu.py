...
class MAINMENU():
    def __init__(self, pypresence):
        ...
        gameTitle = Label(self.menu, text="Snakes & Ladders", font=tkfont.Font(
            family='Helvetica', size=36, weight='bold'))
        gameTitle.pack(anchor='n')
        onePlayer = Button(self.menu, text='1 Joueur', font=tkfont.Font(
            family='Helvetica', size=12, weight='normal'), width=35)
        onePlayer.pack(anchor='n', pady=10)
        twoPlayers = Button(self.menu, text='2 Joueurs', font=tkfont.Font(
            family='Helvetica', size=12, weight='normal'), width=35)
        twoPlayers.pack(anchor='n', pady=10)
        threePlayers = Button(self.menu, text='3 Joueurs', font=tkfont.Font(
            family='Helvetica', size=12, weight='normal'), width=35)
        threePlayers.pack(anchor='n', pady=10)
        fourPlayers = Button(self.menu, text='4 Joueurs', font=tkfont.Font(
            family='Helvetica', size=12, weight='normal'), width=35)
        ...
        if pypresenceEnabled and self.pypresenceRPC is not None:
            self.pypresenceRPC.update(state="Dans les menus !", large_image="snakes-and-ladders",
                                      start=time.time(), large_text="PyLadders&Snakes")
        self.menu.mainloop()
