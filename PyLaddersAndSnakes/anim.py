...
class ANIM():
    def __init__(self, canvas, dice_face, rd):
        ...
        for i in range(random.randint(8, 15)):
            canvas.Terrain.after(100, canvas.Terrain.itemconfigure(
                dice_face, text=random.randint(1, 6), fill="black"))
        ...
        if canvas.playerturn == (len(canvas.player) - 1) and not canvas.win:
            ...
            canvas.Terrain.itemconfigure(
                canvas.playerturnlabel, text="Au tour de " + str(canvas.PlayerNames[0]))
        elif not canvas.win:
            canvas.playerturn += 1
