...
class MAINGAME:
    def __init__(self, PlayerNumber, PlayerNames, PlayerColors, pypresence):
        ...
        self.carreau = [
            [
                self.Terrain.create_rectangle(
                    i * 60, j * 60, (i + 1) * 60, (j + 1) * 60, fill="#FFFFFF"
                )
                for i in range(10)
            ]
            for j in range(10)
        ...
        self.labels = [
            [self.Terrain.create_text(i * 60 + 50, j * 60 + 10) for i in range(10)]
            for j in range(10)
        ]
        self.labels.reverse()
        ...
        if self.playerNumber == 1:
            self.player = [
                self.Terrain.create_oval(
                    10, 550, 50, 590, fill=PlayerColors[0], outline="grey"
                )
            ]
            ...
            self.player = [
                self.Terrain.create_oval(
                    4, 560, 24, 580, fill=PlayerColors[0], outline="grey"
                ),
                self.Terrain.create_oval(
                    36, 560, 56, 580, fill=PlayerColors[1], outline="grey"
                ),
            ]
            if pypresenceEnabled and self.pypresenceRPC is not None:
                self.pypresenceRPC.update(
            ...
            self.player = [
                self.Terrain.create_oval(
                    14, 560, 24, 570, fill=PlayerColors[0], outline="grey"
                ),
                self.Terrain.create_oval(
                    36, 560, 46, 570, fill=PlayerColors[1], outline="grey"
                ),
                self.Terrain.create_oval(
                    24, 580, 34, 590, fill=PlayerColors[2], outline="grey"
                ),
            ]
            if pypresenceEnabled and self.pypresenceRPC is not None:
                self.pypresenceRPC.update(
            ...
            self.player = [
                ...
                self.Terrain.create_oval(
                    14, 580, 24, 590, fill=PlayerColors[2], outline="grey"
                ),
                self.Terrain.create_oval(
                    36, 580, 46, 590, fill=PlayerColors[3], outline="grey"
                ),
            ]
            if pypresenceEnabled and self.pypresenceRPC is not None:
                self.pypresenceRPC.update(
        ...
        self.playerPositionGap = [
            [50],
            [56, 24],
            [46, 24, 36],  # gap entre la position de la case et celle du joueur
            [46, 24, 46, 24],
        ]  # sur l'axe x
        self.playerPositionGapY = [
            [50],
            [40, 40],
            [40, 40, 20],  # gap entre la position de la case et celle du joueur
            [40, 40, 20, 20],
        ]  # sur l'axe y
        ...
        self.dice = self.Terrain.create_rectangle(
            655, 25, 750, 120, fill="white", tags="dice"
        )
        ...
        for i in range(len(objects["ladders"])):  # placement des échelles
            ...
            if objects["ladders"][i][len(objects["ladders"][i]) - 1][1] % 2 == 0:
                objects["endLadders"].append(
                    [
                        objects["simtab"][
                            objects["ladders"][i][len(objects["ladders"][i]) - 1][0]
                        ],
        ...
        for i in range(len(objects["snakes"])):  # placement des serpents
            ...
            if objects["snakes"][i][len(objects["snakes"][i]) - 1][1] % 2 == 0:
                objects["endSnakes"].append(
                    [
                        objects["simtab"][
                            objects["snakes"][i][len(objects["snakes"][i]) - 1][0]
                        ],
            ...
            for j in range(len(objects["snakes"][i])):
                self.Terrain.itemconfigure(
                    self.carreau[objects["snakes"][i][j][1]][
                        objects["snakes"][i][j][0]
                    ],
                    fill="red",
                )
    ...
    def onDiceClick(self, _event):
        ...
        diceAnim = threading.Thread(
            target=ANIM, args=(self, self.dice_face, rd_dice_face)
        )
        diceAnim.start()
    ...
    def moveplayer(self, rd):
        ...
        if objects["playerline"][playerturn] % 2 == 0:
            if (
                self.Terrain.coords(self.player[playerturn])[0] + rd * 60
                <= self.playerPositionY[self.playerNumber - 1][playerturn][0]
                and self.Terrain.coords(self.player[playerturn])[2] + rd * 60
                <= self.playerPositionY[self.playerNumber - 1][playerturn][1]
            ):
                self.Terrain.move(self.player[playerturn], rd * 60, 0)
            else:
                while (
                    self.Terrain.coords(self.player[playerturn])[0]
                    < self.playerPositionY[self.playerNumber - 1][playerturn][0]
                ):
                    self.Terrain.move(self.player[playerturn], 60, 0)
                    cases += 1
                self.Terrain.move(
                    self.player[playerturn], -(abs(((rd - cases) - 1) * 60)), -60
                )
                objects["playerline"][playerturn] += 1
        else:
            if (
                self.Terrain.coords(self.player[playerturn])[1]
                == self.playerPositionYMax[self.playerNumber - 1][playerturn][0]
            ):
                if (
                    self.Terrain.coords(self.player[playerturn])[0] - rd * 60
                    < self.playerPositionYMax[self.playerNumber - 1][playerturn][0]
                    ...
                    while (
                        self.Terrain.coords(self.player[playerturn])[0]
                        > self.playerPositionYMax[self.playerNumber - 1][playerturn][1]
                    ):
                        self.Terrain.move(self.player[playerturn], -60, 0)
                        cases += 1
                ...
                if (
                    self.Terrain.coords(self.player[playerturn])[0] - rd * 60
                    >= self.playerPositionX[self.playerNumber - 1][playerturn][0]
                    and self.Terrain.coords(self.player[playerturn])[2] - rd * 60
                    >= self.playerPositionX[self.playerNumber - 1][playerturn][1]
                ):
                    self.Terrain.move(self.player[playerturn], -(rd * 60), 0)
                else:
                    while (
    ...
    def detectCollision(self):
        ...
        for i in range(len(objects["endSnakes"])):
            if int(self.Terrain.coords(self.player[playerturn])[0]) == 600 - (
                ...
                if int(self.Terrain.coords(self.player[playerturn])[1]) == 600 - (
                    (objects["endSnakes"][i][1] * 60)
                    + self.playerPositionGapY[self.playerNumber - 1][playerturn]
                ):
                    ...
                    self.Terrain.move(
                        ...
                        (
                            600
                            - (
                                (objects["startSnakes"][i][1] * 60)
                                + self.playerPositionGapY[self.playerNumber - 1][
                                    playerturn
                                ]
                            )
                        )
    ...
    def detectWin(self):
        ...
        """
        if (
            int(self.Terrain.coords(self.player[self.playerturn])[0])
            == self.playerPositionX[self.playerNumber - 1][self.playerturn][0]
        ):
            if (
                self.Terrain.coords(self.player[self.playerturn])[1]
                == self.playerPositionYMax[self.playerNumber - 1][self.playerturn][0]
            ):
                self.Terrain.tag_unbind("dice_face", "<Button-1>")
                self.Terrain.tag_unbind("dice", "<Button-1>")
                self.Terrain.itemconfigure(
